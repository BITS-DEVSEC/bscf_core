  ...
  module Core
    RSpec.describe DeliveryOrder, type: :model do
      ...
      describe '#optimized_route' do
        ...
        before do
          # Update the factory or create addresses with coordinates
        ...
        it 'returns an array of item IDs in optimized order' do
          ...
          # Mock different coordinates for testing distance calculation
          ...
          optimized_route = delivery_order.optimized_route

          expect(optimized_route).to be_an(Array)
          expect(optimized_route.size).to eq(2)
          expect(optimized_route).to include(item1.id)
          expect(optimized_route).to include(item2.id)
          # Item1 should come before item2 in the optimized route (closer to pickup)
          expect(optimized_route.index(item1.id)).to be < optimized_route.index(item2.id)
        end
        ...
        it 'returns empty array if pickup address has no coordinates' do
          ...
          expect(delivery_order.optimized_route).to eq([])
        end
