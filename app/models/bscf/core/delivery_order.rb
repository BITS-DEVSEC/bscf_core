  ...
  module Core
    class DeliveryOrder < ApplicationRecord
      ...
      def optimized_route
        ...
        # Call Gebeta Maps service (to be implementee
        gebeta_service = GebetaMapsService.new
        route_data = gebeta_service.optimize_route(pickup_address, dropoff_addresses)

        # Cache the result if needed
        # Rails.cache.write("delivery_order_route_#{id}", route_data, expires_in: 1.hour)e
        route_data
      end

      # Reorder delivery items based on optimized route
      def reorder_items_by_route
